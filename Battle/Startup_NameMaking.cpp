...
public:
  ...
  void  render(Image&  dst, Point  offset) const noexcept override
  {
    ...
    Point  pt;
      ...
      for(int  n = 0;  n < 5;  ++n)
      {
        dst.print(*p++,offset+pt,system_data::glset);
    ...
    dst.print(u'_',offset,system_data::glset);
  }
...
public:
  ...
  void  render(Image&  dst, Point  offset) const noexcept override
  {
    ...
    dst.print(tmp::name_buffer.get_data(),offset,system_data::glset);

      if(tmp::name_buffer.get_length() < length_max)
      {
        offset.x += 8*tmp::name_buffer.get_length();

        dst.print(u'■',offset,system_data::glset);
      }
  }
...
public:
  ...
  void  render(Image&  dst, Point  offset) const noexcept override
  {
    ...
    dst.print("しゅじんこうの　なまえを　きめてください",offset,system_data::glset);

      if(too_short)
      {
        dst.print("なまえが　みじかすぎます",offset.move_x(16).move_y(16),system_data::glset);
      }
  }
...
{
  system_data::root_task.erase(character_window);
  system_data::root_task.erase(     name_window);
...
start_name_making(coreturn_t  ret) noexcept
{
  ...
  system_data::root_task.push(character_window);
  system_data::root_task.push(     name_window);
