...
public:
  constexpr type_info(uint32_t  index, std::string_view  name, std::string_view  id, size_t  size, size_t  align) noexcept:
  m_index(index),
  m_name(name),
  m_id(id),
  m_size(size),
  ...
  constexpr std::string_view    get_id() const noexcept{return   m_id;}
